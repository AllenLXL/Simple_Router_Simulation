...
void sr_arpcache_sweepreqs(struct sr_instance *sr) { 
    /* Fill this in */
    for (struct sr_arpreq* loop_r = sr->request.cache; loop_r; loop_r=loop_r->next){
        handle_arpreq(sr, loop_r);
    }
}
...
void handle_arpreq(struct sr_instance *sr, struct sr_arpreq* req){
    ...
    if (difftime(time(NULL),req->sent)> 1.0){
        if (req->times_sent >= 5){
            ...
            for (struct sr_packet* pkt=req->packets; pkt; pkt=pkt->next){
                // lfm
            ...
            memcpy(e_hdr->ether_dhost, "\xff\xff\xff\xff\xff\xff", ETHER_ADDR_LEN);

            sr_arp_hdr_t* arp_hdr = (sr_arp_hdr_t*) (e_hdr+sizeof(sr_ethernet_hdr_t));
            arp_hdr->ar_hrd = htons(arp_hrd_ethernet);
            arp_hdr->ar_pro = htons(0x0800); // wiki
            arp_hdr->ar_hln = (unsigned char)ETHER_ADDR_LEN;
            arp_hdr->ar_pln = (unsigned char)4; // wiki
            ...
            //2.req->sent = now 3.req->times_sent++
            req->sent = time(NULL);
            req->times_sent++;
        }
    }
